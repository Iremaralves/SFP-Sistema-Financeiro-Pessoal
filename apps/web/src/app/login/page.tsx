...
function traduzirErro(msg: string): string {
  ...
  if (m.includes('email rate limit exceeded'))          return 'Muitas tentativas. Aguarde antes de tentar novamente.';
  if (m.includes('too many requests'))                  return 'Muitas tentativas. Aguarde antes de tentar novamente.';
  if (m.includes('token has expired') ||
      m.includes('is invalid'))                         return 'O link expirou ou é inválido. Solicite um novo.';
  ...
  if (m.includes('signup_disabled'))                    return 'Cadastro de novos usuários desativado.';
  if (m.includes('new password should be different'))   return 'A nova senha deve ser diferente da senha atual.';
  if (m.includes('weak password'))                      return 'Senha fraca. Use letras, números e símbolos.';
  if (m.includes('invalid email'))                      return 'E-mail inválido.';
  if (m.includes('unable to validate email'))           return 'E-mail inválido.';
  // fallback: retorna o original para não perder info em casos não mapeados
  return msg;
}
